...
class UpdateController extends Controller
{
    ...
    public function submitUpdateUser()
    {
        if (EmailExist_2(request()->get('email'), request()->get('id_user')))
            return redirect()->back()->withInput()->with('alert_danger', ' <i> \'utilisateur ayant l\'Email ' . request()->get('email') . '</i> est deja utilisé par un autre utilisateur.');
        else
        {
            $item = User::find(request()->get('id_user'));
            $item->update([
            ...
            return redirect()->route('admin.info', ['p_id' => request()->get('id_user') , 'p_table' => 'users' ])->with('alert_success', 'La Modification de l\'utilisateur s\'est éffectuée avec succès.');
        }
    }
    ...
    public function submitUpdateArticle()
    {
        ...
        return redirect()->route('direct.info', ['p_table' => 'articles', 'id' => request()->get('id_article')])->with('alert_success', 'La Modification de l\'article s\'est éffectuée avec succès.');
    }
    ...
    public function submitUpdateCategorie()
    {
        ...
        return redirect()->route('direct.info', ['p_table' => 'categories', 'id' => request()->get('id_categorie')])->with('alert_success', 'La Modification du fournisseur s\'est éffectuée avec succès.');
    }
    ...
    public function submitUpdateFournisseur()
    {
        ...
        return redirect()->route('direct.info', ['p_table' => 'fournisseurs', 'id' => request()->get('id_fournisseur')])->with('alert_success', 'La Modification du fournisseur s\'est éffectuée avec succès.');
    }
    ...
    public function submitUpdateMagasin()
    {
        ...
        return redirect()->route('direct.info', ['p_tables' => 'magasins', 'id' => request()->get('id_magasin')])->with('alert_success', 'La Modification du magasin s\'est éffectuée avec succès.');
    }
