	...
	*********************************************************/
	...
	public function submitAddUser()
	{
		...
		if( EmailExist( request()->get('email') , 'users' )  )
			return redirect()->back()->withInput()->with('alert_danger',"<li><i> L\'email ".request()->get('email')."</i> est deja utilisé.");
		...
		if( strlen(request()->get('password'))<7 )
			...
			{
				return redirect()->back()->withInput()->with('alert_danger','<strong>Erreur !!</strong> l\'ajout de  l\'utilisateur'.request()->get('email').' a echoue. <br> message d\'erreur: '.$ex->getMessage().'');
			}

		return redirect()->back()->with('alert_success','L\'ajout de l\'utilisateur <strong>'.request()->get('email').'</strong>"  s\'est effectué avec succès');
	}
	...
	public function submitAddMagasin()
	{
		...
		{
			return redirect()->back()->withInput()->with('alert_danger','<strong>Erreur !!</strong> l\'ajout du magasin: '.request()->get('libelle').' a echoue. <br> message d\'erreur: '.$e->getMessage().'');
		}
		return redirect()->back()->with('alert_success','Le Magasin <strong>'.request()->get('libelle').'</strong> a été ajouté avec succès.');
	...
	public function submitAddCategorie()
	{
		...
		{
			return redirect()->back()->withInput()->with('alert_danger','<strong>Erreur !!</strong> l\'ajout de la catégorie '.request()->get('libelle').' a echoue. <br> message d\'erreur: '.$ex->getMessage().'');
		}
		return redirect()->back()->with('alert_success','La Categorie <strong>'.request()->get('libelle').'</strong> a été ajoutée avec succès.');
	}
	...
	public function submitAddFournisseur()
	{
		...
		{
			return redirect()->back()->withInput()->with('alert_danger','<strong>Erreur !!</strong> l\'ajout du fournisseur: '.request()->get('libelle').' a echoue. <br> message d\'erreur: '.$ex->getMessage().'');
		}
		return redirect()->back()->with('alert_success','Le Fournisseur <strong>'.request()->get('code').': '.request()->get('libelle').'</strong> a été ajouté avec succès.');
	...
	public function submitAddArticle()
	{
		...
		if( request()->has('force') && request()->get('force')=="true" )
		{
			...
			return redirect()->back()->with('alert_success','L\'article <strong>'.request()->get('designation_c').'</strong> a été ajouté avec succès.');
		}
		else
		{
			if( Exists('articles', 'designation_c', request()->get('designation_c') )  ) {
			...
			if( Exists('articles', 'code_barre', request()->get('code_barre') )  ){
			    $alerts1 = $alerts1."<li>Le code Barre: <i>".request()->get('code')."</i> est deja utilisé pour un autre article.";
                $error1 = true;
            }
	...
	public function submitAddPromotions()
	{
		...
		for( $i=1; $i<=count($id_article) ; $i++ )
		{
			...
			catch (Exception $e) { $error2 = true; $alert2 = $alert2."<li>Erreur d'ajout de l'article numero ".$i.": <br>Message d'erreur: ".$e->getMessage();}
		...
		return redirect()->back()->with('alert_success','La Création des promotions a été reussite. le nombre d\'articles est : '.$nbre_articles);
	}
